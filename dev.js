...
function load () {
	...
	if (!op.get(page, 'title')) op.set(page, 'title', op.get(metadata, 'title.vi'))
	if (!op.get(page, 'description')) op.set(page, 'description', op.get(metadata, 'description.vi'))
	if (!op.get(page, 'primary_button.text')) {
		op.set(page, 'primary_button.text', op.get(metadata, 'primary_button.vi'))
	}
	if (!op.get(page, 'secondary_button.text')) {
		op.set(page, 'secondary_button.text', op.get(metadata, 'secondary_button.vi'))
	}

	let onClicked = (e) => console.log('click', e)

	let onClosed = () => setTimeout(() => camtemp.reset(), 2000)
